...
const PRODUCT_MAP = {
  // Bebidas sin Alcohol - 40€
  'OR': 'Oráculo Rosa',
  'AD': 'Amanecer Dorado',
  'OM': 'Océano Místico',
  'BL': 'Bruma Lunar',
  'MS': 'Marea Serena',
  // Bebidas con Alcohol - 60€
  'TT': 'Tarot Tonic',
  'AR': 'El Arcano',
  'EM': 'Estrella de Mar',
  'LL': 'Luna Llena',
  // Snacks - 40€
  'DE': 'Dátiles Encantados',
  'TO': 'La Tosta del Oráculo',
...
function showSearchInterface() {
  ...
  <a onclick="window.history.back(); return false;" class="back-home" aria-label="Volver al inicio">
    ...
    <div class="search-container">
      <div class="search-box">
        <img src="assets/logo.png" alt="Logo Beach Club" class="search-logo">
    ...
    <style>
      ...
      .clear-btn:hover {
        ...
        background: linear-gradient(135deg, #d44334 0%, #a93226 100%);
      }
      #recent-list {
        display: flex;
        flex-direction: column;
...
function generateInvoiceNumber() {
  ...
  return `BC-${year}${month}${day}-${hours}${minutes}${seconds}`;
}
...
function populateInvoice() {
  ...
  const urlParams = new URLSearchParams(window.location.search);
  const hasDataParam = urlParams.get('data') !== null;
  const hasCodigoParam = urlParams.get('codigo') !== null;
  const fromSearch = urlParams.get('from') === 'search';

  // Solo es "desde calculadora" si viene con data Y NO es desde búsqueda Y no existía previamente
  // Si ya existía, significa que se está viendo una factura guardada
  const isFromCalculator = hasDataParam && !hasCodigoParam && !fromSearch && !isAlreadySaved;
  ...
  document.title = `Factura ${invoiceNumber} - Beach Club`;
